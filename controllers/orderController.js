...
exports.createOrder = async (req, res) => {
  try {
    ...
    // Convert paymentMethod to lowercase to match enum values
    ...
    // Order confirmation email will be sent after payment is confirmed
    // via /api/payment/confirm endpoint
...
exports.getMyOrders = async (req, res) => {
  try {
    ...
    const syncedOrders = await Promise.all(orders.map(computeAndSyncStages));
...
exports.getOrderById = async (req, res) => {
  try {
    const rawOrder = await Order.findById(req.params.id).populate("user");

    if (!rawOrder) return res.status(404).json({ message: "Order not found" });
    if (rawOrder.user._id.toString() !== req.user._id.toString())
      return res.status(403).json({ message: "Not authorized" });

    const order = await computeAndSyncStages(rawOrder);
    res.json(order);
